...
string trimNewlines(const string &input)
{
    ...
    auto start2 = std::find_if(str.begin(), str.end(), [](unsigned char ch)
                               { return !std::isspace(ch); });
    auto end2 = std::find_if(str.rbegin(), str.rend(), [](unsigned char ch)
                             { return !std::isspace(ch); })
                    .base();
...
string procesarJSON(const string &retornoJson)
{
    try
    {
        ...
        {
            string errorType = data["error"];
            string message = data["message"];
            if (errorType == "connection_error")
            {
                return "Lo siento, no pude conectarme al servidor de IA. Por favor, verifica que el modelo esté ejecutándose correctamente.";
            }
            else if (errorType == "http_error")
            {
                return "Error del servidor de IA: " + message;
            }
            else
            {
...
void ejecutarServidor(const Config &config)
{
    ...
    cout << "Iniciando servidor con comando: " << comando << endl;

    if (CreateProcess(NULL, (LPSTR)comando.c_str(), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
    {
        servidorProcess = pi.hProcess; // Guardamos el handle del proceso
        CloseHandle(pi.hThread);       // No necesitamos el handle del hilo
        cout << "Servidor iniciado exitosamente." << endl;
    }
    else
    {
        DWORD error = GetLastError();
        cerr << "Error al iniciar el servidor. Código de error: " << error << endl;
    }
}
...
string interactuarConModelo(const string &prompt, const Config &config)
{
    ...
    nlohmann::json jsonBody = {
        ...
        {"max_tokens", 400},
        {"temperature", 0.7},
        {"stop", nlohmann::json::array({"</s>", "HARRY:", "USUARIO:"})},
        {"repeat_last_n", 256},
        {"repeat_penalty", 1.18},
        {"top_k", 40},
        {"top_p", 0.95},
        ...
        {"api_key", ""},
        {"slot_id", -1},
        {"prompt", prompt} // Asumiendo que 'prompt' es una variable std::string
    };
