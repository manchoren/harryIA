...
static int handler(void* user, const char* section, const char* name, const char* value) {
    ...
    if (strcmp(name, "EJECUTABLE") == 0) config->EJECUTABLE = value;
    else if (strcmp(name, "HABILITAR_SERVIDOR") == 0) config->HABILITAR_SERVIDOR = stoi(value);
    else if (strcmp(name, "MODO_SERVIDOR") == 0) config->MODO_SERVIDOR = value;
    else if (strcmp(name, "PARAMETROS_EXTRA") == 0) config->PARAMETROS_EXTRA = value;
    else if (strcmp(name, "API_URL") == 0) config->API_URL = value;
    else if (strcmp(name, "API_ENDPOINT") == 0) config->API_ENDPOINT = value;
    else if (strcmp(name, "TIEMPO_ESPERA_SERVIDOR") == 0) config->TIEMPO_ESPERA_SERVIDOR = stoi(value);
    else if (strcmp(name, "LOG_FILE") == 0) config->LOG_FILE = value;
    else if (strcmp(name, "MEMORY_FILE") == 0) config->MEMORY_FILE = value;
    else if (strcmp(name, "START_FILE") == 0) config->START_FILE = value;
    else if (strcmp(name, "PIPER_EXE") == 0) config->PIPER_EXE = value; 
    else if (strcmp(name, "PIPER_MODEL") == 0) config->PIPER_MODEL = value; 
    else if (strcmp(name, "VLC_EXE") == 0) config->VLC_EXE = value; 
    else if (strcmp(name, "VLC_EXTRA_PARAMS") == 0) config->VLC_EXTRA_PARAMS = value; 
    else if (strcmp(name, "DEBUG") == 0) config->DEBUG = stoi(value);
    return 1;
}
